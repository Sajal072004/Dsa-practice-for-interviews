...
ll findAns(int divisor, vector<ll>&arr, vector<ll>&pre){
  ll mini = pre[divisor-1]; 
  ll maxi = pre[divisor-1];

  for(ll i = 2*divisor-1; i<arr.size(); i+=divisor){
    ll curr = pre[i] - pre[i-divisor];
    mini = min(mini, curr);
    maxi = max(maxi, curr);
  }
  return maxi-mini;
...
void solve()
{
  ...
  for(auto it : answers){
    ll divisor = it;
    ll currAns = findAns(divisor, arr,pre);
    ans = max(ans, currAns);
  }
  cout << ans << endl;
}
