...
int smallerLeft(int i , vector<long long>&arr ,int val){
  while(i>=0){
...
int smallerRight(int j , vector<long long>arr , int val){
  while(j<arr.size()){
...
int main(){
    ...
    while(t--){
      ...
      for(int i = 1; i<n-1; i++){
        int left = smallerLeft(i-1,arr, arr[i]);
        int right = smallerRight(i+1,arr , arr[i]);

        if(left != -1 && right != -1) {
          cout << "YES" << endl; flag = true;
          cout << left+1 << " " << i+1 << " " << right+1 << endl; break;
        }
      }
