...
void solve() {
  ...
  vector<pair<ll, ll>> bList, cList;
  for (ll i = 1; i <= n; i++) bList.push_back({b[i], i});
  for (ll i = 1; i <= n; i++) cList.push_back({c[i], i});

  sort(bList.rbegin(), bList.rend());
  sort(cList.rbegin(), cList.rend());

  ll ans = 0;

  for (ll i = 1; i <= n; i++) {
    for (int bi = 0; bi < min(5LL, n); bi++) {
      if (bList[bi].second == i) continue;
      for (int ci = 0; ci < min(5LL, n); ci++) {
        ll j = bList[bi].second;
        ll k = cList[ci].second;
        if (k == i || k == j) continue;
        ll total = a[i] + b[j] + c[k];
        ans = max(ans, total);
      }
    }
  }
